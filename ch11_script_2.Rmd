...
poverty_model <- lm(county$poverty ~ county$less_than_hs)
tidy(poverty_model, conf.int = TRUE)
```
...
```{r slope, echo=FALSE}
x <- bootstrap %>%
  mutate(mod = map(data, ~ lm(poverty ~ less_than_hs, data = county)),
         reg_results = map(mod, ~ tidy(.)),
         disp_coef = map_dbl(reg_results, ~ filter(., term == "less_than_hs") %>%
                               pull(estimate)))
```
...
By approximately 1.57. Standard error?
...
```{r}
regression_stats <- poverty_model %>%
  augment()
```
# Scene 16
...
```{r}
regression_stats %>%
  mutate(conf_high = 1.96 + .se.fit,
         conf_low = .se.fit - 1.96) 
```
